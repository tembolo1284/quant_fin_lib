...
TEST(QuantFinTest, BinomialTree) {
    ...
    double time = 1;
    double volatility = 0.2;
    int steps = 1000;  // Reduced number of steps for better numerical stability
    
    double call_price = qf_binomial_tree_price(spot, strike, rate, time, volatility, 1, steps);
    double put_price = qf_binomial_tree_price(spot, strike, rate, time, volatility, 0, steps);
    
    // Using a slightly larger tolerance for binomial tree
    EXPECT_TRUE(approximately_equal(call_price, 10.4506, 1e-1));
    EXPECT_TRUE(approximately_equal(put_price, 5.5735, 1e-1));
}
